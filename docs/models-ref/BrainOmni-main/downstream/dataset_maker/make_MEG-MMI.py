...
DATASET_NAME = "MEG-MMI"
DATASET_PATH = os.path.join(EVALUATE_PATH, "ds003568-1.0.4")
...
def preprocess_BrainOmni(
    ...
    exclude = ["MRO11-1609", "MLP33-1609", "MLP54-1609"]
    ...
    data = raw.get_data()
    start = 0
    end = int(SAMPLE_RATE * 30)
    stride_length = int(SAMPLE_RATE *30)
    ...
    while end < data.shape[1]:
        ...
        assert accept_segment(seg_data, pos), f"nan error in {raw_path}"
...
def process(accessor: DataAccessor, method: str, logger, multi_process=True):
    ...
    file_path_list = []
    for root, dirs, names in os.walk(DATASET_PATH):
        if len(dirs) > 0:
            file_path_list += [
                ...
                if (
                    dir.split(".")[-1] == "ds"
                    and "task" in dir
                    and "task-rest" not in dir
                )
            ]

    participants_df = pd.read_csv(
        os.path.join(DATASET_PATH, "participants.tsv"), sep="\t"
    )
    label_dic = {}
    for _, row in participants_df.iterrows():
        id = row["participant_id"]
        label = 1 if row["group"] == "MDD" else 0
        label_dic[id] = label
    ...
    if multi_process:
        with ProcessPoolExecutor(max_workers=16) as executor:
            ...
            for file_path in file_path_list:
                sub = os.path.basename(file_path).split("_")[0]
                futures.append(
                    executor.submit(
        ...
        for file_path in file_path_list:
            ...
            try:
                result = method_preprocess_dict[method](
                    accessor, file_path, label_dic[sub], processed_path
                )

...
DATASET_NAME = "ad65"
DATASET_PATH = os.path.join(EVALUATE_PATH, "ds004504-1.0.8")
...
def process(accessor: DataAccessor, method: str, logger, multi_process=True):
    ...
    for root, dir, names in os.walk(DATASET_PATH):
        if len(names) > 0:
            file_path_list += [
                os.path.join(root, name) for name in names if name.endswith(".set")
            ]
    ...
    for row in label_data:
        label_dic[row[0]] = LABEL_MAP[row[3]]
        ...
        for file_path in file_path_list:
            ...
            try:
                result = method_preprocess_dict[method](
                    accessor, file_path, label_dic[subject], processed_path
                )
...
if __name__ == "__main__":
    ...
    for method in ["BrainOmni"]:
        ...
        # only use healthy and alz
        metadata_list = [i for i in metadata_list if i["label"] != 1]
        for i in range(len(metadata_list)):
            metadata_list[i]["label"] = min(1, metadata_list[i]["label"])

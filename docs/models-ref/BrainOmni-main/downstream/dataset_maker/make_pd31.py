...
DATASET_NAME = "pd31"
DATASET_PATH = os.path.join(EVALUATE_PATH, "ds002778-1.0.5")
    ...
    if multi_process:
            ...
            for file_path in file_path_list:
                futures.append(
                    executor.submit(
                        ...
                        accessor,
                        file_path,
                        1 if "sub-pd" in file_path else 0,
                        processed_path,
                    )
                )
        ...
        for file_path in file_path_list:
            result = method_preprocess_dict[method](
                accessor, file_path, 1 if "sub-pd" in file_path else 0, processed_path
            )
            try:

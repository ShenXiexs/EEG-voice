...
if ! "$PY" -c 'import speechbrain' >/dev/null 2>&1; then
  echo "[v3] missing SpeechBrain for the audio-only V2 ECAPA oracle. Run: ./bootstrap_open_vocab_v3.sh"
  exit 1
fi
"$PY" -c 'import numpy, scipy, speechbrain, torch, transformers; print({"speechbrain": speechbrain.__version__, "torch": torch.__version__, "mps": torch.backends.mps.is_available(), "cuda": torch.cuda.is_available(), "transformers": transformers.__version__})'
...
# 0. Read-only audio preparation/audit plus V0.  V0 is the first hard stop.
...
# 4. These are reports, not extra optimization.  They are unreachable unless

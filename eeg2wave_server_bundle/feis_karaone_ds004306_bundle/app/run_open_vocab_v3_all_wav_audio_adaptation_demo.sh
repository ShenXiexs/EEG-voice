...
cd "$APP_DIR"
CACHE="$APP_DIR/../artifacts/open_vocab_v3_mfcc_training_first/cache/prepared_mfcc_v3.npz"
if [[ ! -f "$CACHE" ]]; then
...
"$PY" scripts/finetune_open_vocab_v3_audio_models.py \
  --config "$CFG" --scope all --device "${TRAIN_DEVICE:-auto}"

OUTPUT="$APP_DIR/../artifacts/open_vocab_v3_mfcc_training_first/audio_adaptation/transductive_all"

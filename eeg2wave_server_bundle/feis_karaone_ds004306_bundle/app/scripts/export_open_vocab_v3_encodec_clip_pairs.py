...
def parse():
 p=argparse.ArgumentParser();p.add_argument('--config',type=Path,required=True);p.add_argument('--stage',choices=('micro','fit','final'),default='fit');p.add_argument('--device',default='cpu');p.add_argument('--resume',action='store_true');p.add_argument('--max-pairs',type=int,default=0,help='export at most this many pairs for a smoke check; 0 means all');p.add_argument('--output-root',type=Path,default=None,help='optional isolated destination, intended for smoke exports');p.add_argument('--explore',action='store_true',help='allow final export without the training-WAV approval; label outputs exploratory');return p.parse_args()
...
def main():
 ...
 vocoder=SpeechT5HifiGan.from_pretrained(str(output_path(cp,cfg,'vocoder_adapted_root')),local_files_only=True).to(device).eval();codec=EnCodecGenerator(output_path(cp,cfg,'encodec_adapted_root'),device=device,bandwidth=float(cfg['audio']['encodec_bandwidth']));print('[v3 export] loaded adapted EnCodec and HiFi-GAN',flush=True);fit=(records.roles=='fit')&records.arrays['fit_eligible'].astype(bool);fixed_frames=int(np.median(records.arrays['speech_t5_mel_mask'][fit].sum(1)));fixed_samples=fixed_frames*int(cfg['vocoder']['hop_length']);root=a.output_root if a.output_root is not None else (output_path(cp,cfg,'micro_preview_root') if a.stage=='micro' else output_path(cp,cfg,'pair_root') if a.stage=='final' else output_path(cp,cfg,'fit_preview_root'));root.mkdir(parents=True,exist_ok=True);manifest=[]
 ...
 for batch in DataLoader(ds,batch_size=1,shuffle=False,collate_fn=token_collate,num_workers=0):
  ...
  codes,code_mask=codec.encode(torch.from_numpy(reference).unsqueeze(0));valid=int(code_mask[0].sum());codec_oracle=codec.decode(codes[:,:,:valid],target_samples_16k=len(reference))[0].detach().cpu().numpy();oracle,mel_oracle=render(oracle_mfcc);target,mel_target=render(b['mfcc'].float());target_voice,_=render(b['mfcc'].float(),b['speaker_reference'].float(),b['speaker_reference_mfcc_mean'].float(),b['speaker_reference_mfcc_std'].float());pred,mel_pred=render(correct);z,mel_z=render(zero);t,mel_t=render(time);c,mel_c=render(channel);oracle=oracle[:fixed_samples];target=target[:fixed_samples];pred=pred[:fixed_samples];z=z[:fixed_samples];t=t[:fixed_samples];c=c[:fixed_samples];target_voice=target_voice[:len(reference)]
 ...
 name='export_manifest.json' if a.stage=='final' else 'preview_manifest.json';lineage=capture_lineage(cp,cfg,artifact_keys=(f'{checkpoint_stage}_checkpoint','fit_gate') if checkpoint_stage=='fit' else (f'{checkpoint_stage}_checkpoint','micro_gate'));complete=not a.max_pairs or len(manifest)==len(ds);write_json(root/name,{'schema_version':'openvoice-v3-encodec-clip-preview-v1','stage':a.stage,'exploratory_gate_bypass':bool(a.explore),'complete':complete,'n':len(manifest),'lineage':lineage,'pairs':manifest});
 if a.stage=='final':

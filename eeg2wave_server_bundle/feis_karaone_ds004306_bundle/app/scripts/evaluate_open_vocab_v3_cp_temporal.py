...
def gate_cvae(cp,cfg,records,device):
    dataset=selected_audio(records,cfg,dev=True,per_label=int(cfg["evaluation"]["oracle_per_label"]));_,_,_,_,mel_teacher,cvae=make_modules(cfg,device);raw=load_checkpoint(output_path(cp,cfg,"cvae_checkpoint"),checkpoint_schema(cfg,"cvae"),{"cvae":cvae,"teacher":mel_teacher},device);target=[];det=[];prior=[];post=[];samples=[[] for _ in range(int(cfg["evaluation"]["variational_samples"]))];activity=[]
    with torch.inference_mode():
    ...
    numerical_fallback=bool(raw.get("extra",{}).get("numerical_fallback",False));checks={"finite_training_checkpoint":not numerical_fallback,"label":wav_metrics["prior"]["label_top1"]>=g["label_top1_min"],"dtw_retention":dtw_drop<=g["dtw_drop_max"],"posterior":post_improvement>=g["posterior_improvement_min"],"prior_posterior":abs(wav_metrics["prior"]["label_top1"]-wav_metrics["posterior"]["label_top1"])<=g["prior_posterior_label_gap_max"],"sample_content_retention":all(value["label_top1"]>=g["label_top1_min"] for value in sample_wav_metrics),"residual_budget":residual_ratio<=g["residual_rms_ratio_max"],"diversity_low":diversity>=g["diversity_ratio_min"],"diversity_high":diversity<=g["diversity_ratio_max"],"inactive":inactive_active<=g["inactive_active_residual_ratio_max"]}
    return {"gate":"T2V","n":len(labels),"numerical_fallback_checkpoint":numerical_fallback,"metrics":{"mel_retrieval":{"deterministic":de,"prior":pr,"posterior":po},"independent_hubert_wav":wav_metrics,"prior_sample_hubert_wav":sample_wav_metrics,"dtw_hubert_drop":dtw_drop,"posterior_improvement":post_improvement,"residual_rms_ratio":residual_ratio,"prior_diversity_ratio":diversity,"inactive_active_residual_ratio":inactive_active},"thresholds":g,"checks":checks}
